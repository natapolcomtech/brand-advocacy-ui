	...
	<div class="sidebar-header flex items-center justify-between">
		<div class="logo-box grow" :class="{ mini: logoMini }">
			<Transition name="fade" mode="out-in" >
				<div v-if="isDark && !logoMini" class="text-white flex flex-row items-center justify-center gap-2">
					<Logo :mini="false" :dark="true" class="anim-wrap" title="BA App"/>
			
				</div>
				<div v-else-if="isLight && !logoMini" class="text-white flex flex-row items-center justify-center gap-2">
					<Logo :mini="false" :dark="false" class="anim-wrap" title="BA App"/>
				</div>
		...
		<Transition name="fade" mode="out-in">
			<div class="sidebar-pin flex items-center" v-if="showPin">
				<Icon :size="20" @click="sidebarCollapsed = !sidebarCollapsed" color="#ffffff">
					<span class="i-large">
